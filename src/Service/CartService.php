...
class CartService
{
    ...
    public function add(int $id)
    {
        ...
        $stocks = $this->productRepository->find($id)->getStock(); // récupère le stock en base de données
        if (!empty($cart[$id])) { // si le produit est déjà dans le panier
            if ($cart[$id] >= $stocks) { // s'il n'y a pas assez de produits en stock
                return false;
            } else {
                $cart[$id]++; // incrémente de 1 la quantité associée
            }
        } else {
            if ($stocks > 0) { // s'il y a un moins 1 produit en stock
